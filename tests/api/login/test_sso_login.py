...
from schema.operations import SSOLoginRequestSchema, MessageErrorSchema
...
class TestSSOLogin:
    ...
    async def test_sso_invalid_code_returns_400(self, api_client: APIClient) -> None:
        ...
        response = await api_client.sso_login(payload)
    ...
    async def test_sso_open_redirect_rejected(self, api_client: APIClient) -> None:
        payload = SSOLoginRequestSchema(
            orgName="acme-corp",
            code="some_code",
    ...
    @pytest.mark.slow
    async def test_sso_rate_limit_429(self, api_client: APIClient) -> None:
        status_codes = []
        for _ in range(55):
            r = await api_client.post("/sso_login", json={
                "orgName": "acme-corp",
                "code": "some_code",
                "redirect_uri": "https://app.kalsense.com/auth/callback",
                "provider": "google",
    ...
    async def test_sso_sql_injection(self, api_client: APIClient) -> None:
        payload = SSOLoginRequestSchema(
            orgName="' OR '1'='1",
            code="some_code",
            redirect_uri="https://app.kalsense.com/auth/callback",
    ...
    async def test_sso_no_token_in_error(self, api_client: APIClient) -> None:
        payload = SSOLoginRequestSchema(
            orgName="acme-corp",
            code="invalid_code",
            redirect_uri="https://app.kalsense.com/auth/callback",
