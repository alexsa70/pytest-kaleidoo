...
from clients.org_client import OrgClient
...
class TestCreateOrgPositive:
    ...
    async def test_create_org_required_fields(
        self, org_client: OrgClient, settings: Settings,
    ) -> None:
        payload = base_org_payload(settings)
        response = await org_client.create_org(payload)
    ...
    async def test_create_org_valid_colors(
        self, org_client: OrgClient, settings: Settings, color: str,
    ) -> None:
        payload = base_org_payload(settings)
    ...
    async def test_create_org_with_png_logo(
        ...
        payload = base_org_payload(settings)
        response = await org_client.create_org(
            payload,
...
class TestCreateOrgValidation:
    ...
    async def test_create_org_duplicate_name(
        ...
        await org_client.create_org(payload)

        payload2 = base_org_payload(settings)
        payload2.org_name = "duplicate-org-test-fixed"
        response = await org_client.create_org(payload2)
    ...
    async def test_create_org_missing_required_field(
        self, org_client: OrgClient, settings: Settings, field: str,
    ) -> None:
        data = base_org_payload(settings).model_dump(exclude_none=True)
        del data[field]
        response = await org_client.post("/api/org/create", data=data)
        assert response.status_code in (
            HTTPStatus.BAD_REQUEST.value,
            HTTPStatus.UNPROCESSABLE_ENTITY.value,
        )
...
class TestCreateOrgAuthorization:
    ...
    async def test_create_org_no_token(
        self, api_client_no_auth: AuthClient, settings: Settings,
    ) -> None:
        payload = base_org_payload(settings)
    ...
    async def test_create_org_invalid_token(
        ...
        response = await org_client.post(
            "/api/org/create",
            data=payload.model_dump(exclude_none=True, exclude={"permissions", "supported_languages"}),
